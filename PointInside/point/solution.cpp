...
bool is_inside(const point& p, const rectangle& r)
{
    int leftx = r.origin.x;
    int rightx = r.origin.x + r.width;
    int topedgey = r.origin.y;
    int bottomedgey = r.origin.y + r.height;
    
    if  (leftx <= p.x && p.x <= rightx && topedgey <= p.y && p.y <= bottomedgey)
        {
