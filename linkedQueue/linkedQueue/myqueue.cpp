...
{
    if (x.q_front == nullptr)
           return;

       // Copy each node from the other queue to the current one
       node* ptr = x.q_front;
       while (ptr != nullptr)
       {
           push(ptr->value); // Use push to add new elements to the back of the current queue
           ptr = ptr->next;
       }
