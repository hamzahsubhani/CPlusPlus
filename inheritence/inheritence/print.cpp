...
int main()
{
    ...
    read_hitters("/Users/hamzahsubhani/Documents/GitHub/CPlusPlus/inheritence/inheritence/hitters.txt", roster);
    read_pitchers("/Users/hamzahsubhani/Documents/GitHub/CPlusPlus/inheritence/inheritence/pitchers.txt", roster);
...
void print_hitters(const vector<player*>& roster)
{
    for (size_t i = 0; i < roster.size(); i++)
    {
        const player* p = roster[i];
        const hitter* valid_hitter = dynamic_cast<const hitter*>(p); // a valid hitter is one that does not point to nullptr when downcasting
        if (valid_hitter !=nullptr)
        {
            valid_hitter -> print();
        }
    }
...
void print_pitchers(const vector<player*>& roster)
{
    ...
    {
        const player* p = roster[i];
        const pitcher* p_pitcher = dynamic_cast<const pitcher*>(p);
        if (p_pitcher!= nullptr)
        {
            p -> print();
        }
    }
