...
{
    size_t min_capacity = (v_capacity < rhs.v_capacity) ? v_capacity : rhs.v_capacity;
    vectorn result;
    result.v_capacity = min_capacity;
    result.v_array = new double[min_capacity];

    for (size_t i = 0; i < min_capacity; i++)
    {
        result.v_array[i] = v_array[i] + rhs.v_array[i];
    }
...
double vectorn::operator*(const vectorn& rhs) const
{
    ...
    for ( size_t i = 0; i < min_capacity; i++)
    {
        dot_product += v_array[i] * rhs.v_array[i];
    }
...
vectorn vectorn::operator*(double rhs) const
{
    ...
    result.v_array = new double[v_capacity];
