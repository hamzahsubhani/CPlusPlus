...
int insert_into_sorted(int source[], int source_len, int destination[], int new_elem)
{
    ...
    while (i < source_len && source[i] < new_elem) {
        i++;
    }
    
    // Copy elements from source to destination up to the insertion point
    for (j = 0; j < i; j++) {
    ...
    // Copy remaining elements from source to destination
    for (j = i; j < source_len; j++) {
