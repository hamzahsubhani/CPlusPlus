...
void sorted_list::insert(int value)
{
    ...
    {
        previous = curr; // traverse linked list
        curr = curr -> next; // move to next node for sortedness
        
    }
    if (previous == nullptr)
    {
        new_node->next = l_front; // insert at front
        l_front = new_node;
    }
    
    else
    {
        previous -> next = new_node; // insert at back
        new_node -> next = curr;
    }
    l_size++;
}
...
void sorted_list::remove(int value)
{
    ...
    node* curr = l_front; // one at front and back
    node* previous = nullptr;
    ...
    while (curr!= nullptr)
    {
        if (curr -> value == value)
        {
            node* del_node = curr; // assign value to current value to remove upon match
            ...
            l_size--; // decrease list size upon removal
        }
        else
        {
            previous = curr; // move up
